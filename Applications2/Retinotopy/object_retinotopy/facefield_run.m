function stim = facefield_run(scriptName, subject, varargin);
% stim = facefield_run([scriptName, subject, options]);
%
% Run a face field retinotopy stimulus, based on a script.
%
% SEE: faceret_makescript, saveFaceBarImages, facebar.
%
% ras, 07/15/2008. 
if notDefined('scriptName')
	scriptName = 'scale_factor_2_1.txt';
end
...
stim.responseKeys = {'4' '3' '2'}; % keys for colors in fixation task
stim.runPriority = 7;
...
tim.display.fixType = 'disk';
stim.display.fixGrid = 1;

%% read in the script
stim = faceret_readscript(stim, scriptName);
...
% /------------------------------------------------------------/ %
function stim = faceret_readscript(stim, scriptName);
...
while ~feof(fid)
	...
	if ~strncmp(vals{5}, 'blank', 5)
		...
		pth = explode(sep, vals{5});
		
		% find the entry for 'FaceFieldImages'
		iStart = strmatch('FaceFieldImages', pth);
		if isempty(iStart)
			error('Invalid path.')
		end
...
defColors = [0 0 0; 0 0 1; 1 0 1; 1 1 0; 1 0 0];
...
stim.taskStr = 'Fixation color: (1) blue (2) magenta (3) yellow';
