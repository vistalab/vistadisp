function stim = faceret_makescript(varargin);
%
% stim  = faceret_makescript(varargin);
%
% Create a series of scripts for face bar retinotopy experiments. These scripts
% provide a description of each face bar experiment, down to the precise
...
% input arguments.
%
% Before running this, you must run saveFaceBarImages, which generates sets
% of images containing face bars. 
%
...
%	facebar_noblanks_[set].txt
%	facebar_blanks_[set].txt
% 
...
%% params
set = 1;    % image set from saveFaceBarImages
displayHz = 60; % screen refreshes/s for the display: determines timing
secsPerBlock = 12 * 2.49; % seconds per 'block': each block is 1 bar sweep
...
backAndForth = 0;  % flag for back-and-forth face motion at each position
...
imageDir = fullfile(codeDir, 'FaceBarImages', sprintf('Set%i', set));
...
for n = 1:length(directions)	
	for p = 1:nPositions
		...
		if backAndForth==1
			...
			% if we play all the images in order, the faces will always precess
...
Ioff = find( mod(stim(2).onset-prestimSecs, secsPerBlankCycle) > cutoff | ...
		     mod(stim(2).onset-prestimSecs, secsPerBlankCycle) == 0 );
...
scriptPath = fullfile(scriptDir, sprintf('facebar_noblanks_%i.txt', set));
writeScript(stim(1), scriptPath);

scriptPath = fullfile(scriptDir, sprintf('facebar_blanks_%i.txt', set));
