function stim = slidingFace_run(scriptName, subject, varargin);
% stim = slidingFace_run([scriptName, subject, options]);
%
% Run the single-bar position face size test.
%
% SEE: slidingFace_makescript, slidingFace_saveImages.
%
% ras, 07/15/2008. 
if notDefined('scriptName')
	scriptName = 'sliding_face_1.txt';
end
...
%% parameters
stim.task = 'gender'; % 'gender' or 'position'
stim.blockShuffle = 0;
stim.addBlanks = 0;
...
stim.nRunRepeats = 1;
stim.TR = 1.5;
stim.startScan = 4 * stim.TR; % when do we send the trigger to start scanning?
stim.fixOffset = 0; % X-offset of fixation dot from screen center, in pixels
stim.nFixColors = 2;  % # colors for fixation task
stim.bgColor = 165;
...
stim.display.fixX = round(res.width/2) + stim.fixOffset;
stim.display.fixY = round(res.height/2);
stim.display.fixSizePixels = 3;
tim.display.fixType = 'dot';
stim.display.fixGrid = 0;

%% read in the script
stim = slidingFace_readscript(stim, scriptName);

%% if the user requests, shuffle the order of face images within each
%% half-cycle:
if stim.blockShuffle==1
    stim.task = 'position'; % gender task is much too hard here
    stim = doBlockShuffle(stim);
end

%% if the user requests, add blank periods
if stim.addBlanks==1
    stim = addBlanks(stim);
end
...
% fixSeq, etc.
stim = slidingFace_stimParams(stim);
...
% /------------------------------------------------------------/ %
function stim = slidingFace_readscript(stim, scriptName);
...
while ~feof(fid)
	...
	stim.posCycle(end+1) = str2num(vals{1});
	stim.seqtiming(end+1) = str2num(vals{2});
	stim.fixSeq(end+1) = 1;  % dummy value
    stim.idCycle(end+1) = str2num(vals{3});
    stim.cond(end+1) = str2num(vals{4});
    stim.pos(end+1) = str2num(vals{5});
	...
	if ~strncmp(vals{7}, 'blank', 5)
		...
		% find the entry for 'SlidingFaceImages'
		iStart = strmatch('SlidingFaceImages', pth);
		if isempty(iStart)
			error('Invalid path.')
		end
...
% /------------------------------------------------------------/ %
function stim = slidingFace_stimParams(stim);
...
switch lower(stim.task)
    ...
    case 'position'
        stim.taskStr = {'What POSITION is the center of the face?'; ...
                        '(1) LEFT of the dot (2) RIGHT of the dot'; ...
                        '(3) center or blank'; ...
                        'Respond only when the the dot blinks red.'};
    otherwise
        error('Unrecognized task: %s.', stim.task)
end
...
% /----------------------------------------------------------------/ %
function stim = doBlockShuffle(stim);
%% temporally shuffle the order of images in each half-cycle. This creates a
%% simple ABAB block design stimulus, where the first half of each cycle is
%% one block, with images of the left side of faces in a randomized order,
%% and the second half is images of the right side of faces in a randomized
%% order. For the A blocks, faces will appear centered in the left visual
%% field, and for the B blocks, faces will appear centered in the right
%% visual field. However, because the image order is shufled, the percept that 
%% there is a face behind the bar is substantially weakened.  
%%
%% This shuffling tests whether any selectivity we may see in the main
%% experiment is a result of the visual face parts we presented, or the
%% subjects' perception that the face lies in the LVF/RVF.

%% find indices in the stim struct for events in the first and second half
%% of each cycle.
firstHalf = [];
secondHalf = [];
for n = 1:max(stim.posCycle)-1
   I = find(stim.posCycle==n);
   half = ceil( length(I) / 2 );
   
   firstHalf = [firstHalf I(1:half)];
   secondHalf = [secondHalf I(half+1:end)];
end

%% to maximize the variation between face images, we shuffle the images
%% from the first half of _all_ cycles together, and the second half of all
%% cycles together.
stim.shuffledImageOrderA = shuffle(1:length(firstHalf));
stim.shuffledImageOrderB = shuffle(1:length(secondHalf));

seqA = firstHalf( stim.shuffledImageOrderA );
seqB = secondHalf( stim.shuffledImageOrderB );

stim.seq(firstHalf) = stim.seq(seqA);
stim.seq(secondHalf) = stim.seq(seqB);
...
% /----------------------------------------------------------------/ %
function stim = addBlanks(stim);
%% set a 12-second blank period 4 times in the main cycles.
%% This serves as a positive control in case we get a null result during
%% the main runs. With this design, a 4-cycles-per-scan corAnal on the main
%% cycles should show stimulus > blank activity in retions which are
%% retinotopically sensitive to the fovea, lower vertical meridian, and
%% upper vertical meridian (where the face aperture is shown).
N = max(stim.posCycle-1); % main cycles

blankIndex = stim.seq(end); % index to blank images

mainCycles = find(stim.posCycle>=1 & stim.posCycle <=N);
onsets = stim.seqtiming(mainCycles);

totalDur = onsets(end) - onsets(1);
onOffCycleDur = totalDur / 4; % should be integer

indicesToSetBlank = (mod(onsets, onOffCycleDur) < 12);

I = mainCycles(indicesToSetBlank);

stim.seq(I) = blankIndex;
...
try,
    ...
    for n = 1:stim.nRunRepeats,
        ...
        % wait for go signal
        pressKey2Begin(stim.display, 0, [], stim.taskStr);      
...
% 	stim = computeGenderTaskPerformance(stim);
    ...
    ShowCursor;
    tmp = lasterror;
    warning('Error in closing windows: %s', tmp.message)
	for n = 1:length(tmp.stack)
        fprintf('%s line %i\n', tmp.stack(n).file, tmp.stack(n).line);
    end
end;
