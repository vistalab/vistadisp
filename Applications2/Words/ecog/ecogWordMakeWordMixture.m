function savedTrialsDir = ecogWordMakeWordMixture(params,loadParfileFlag)
...
%
% ported from TMSmakeMotionWordsRun by amr on Jan 6, 2010
%
%
...
params.movie.display = loadDisplayParams('displayName', 'gunjou.mat');
...
if ~isfield(params,'taskType'), params.taskType = input('Which type of task?  (1) lexical decision  (2) fixation  (determines RT length only):  '); end
if params.taskType == 1
    params.RT = 2.7;   % time after stimulus display to allow user to respond; ITI is in addition to the RT between trials
elseif params.taskType == 2
    params.RT = 0.9;
end
% Typical values are 2.7 for word/nonword and 0.9 for fixation dot task,
% but these can be adjusted based on how subject is doing


% Experimental Parameters common to both modes (pre-generated parfile and stimOrder file)
params.movie.duration = 0.2;  %0.5     % set stimulus duration manually here (overwrite from initWordParams)
...
%params.RT = 0.9;  %2.7 for word/nonword  %0.9 for fixation dot task
%% time after stimulus display to allow user to respond; ITI is in addition to the RT between trials
...
if ~loadParfileFlag  % then create parfile from scratch using a stimOrderFile
    ...
    meanITI = 100;  % 1000                  % in millisecs; only used for poisson distribution way of generating ITIs
    minITI = 25;  % 300                 % useful for rand ITI generation; if using poisson, make sure to set very low for accurate poisson stats
    maxITI = 3000;  % 5000                % useful for rand ITI generation; if using poisson, set very high to allow accurate poisson distribution
...
% %     % Save out stimuli as pictures (e.g. for making presentation slides)
%     prntImg = trialInfo.images{1}./255;
%     figure; imagesc(prntImg); colormap gray; truesize; axis off;
%     imgFileName = sprintf('~/Desktop/ECoGstimuli/Figure%d.tiff',trialInfo.condition);
%     set(gcf,'Color',[0.5 0.5 0.5])
%     [cdata, colorMap] = getframe(gcf); %#ok - colorMap unneeded
%     imwrite(cdata,imgFileName);
%     close(gcf)
