function savedTrialsDir = ecogWordMakeWordImagery(params,loadParfileFlag)
...
%
%  savedTrialsDir = ecogWordMakeWordImagery(params,loadParfileFlag)
%
% same code as ecogWordMakeWordMixture by amr on July 28, 2010-- just some
...
%   In this case, we're just reading in some filenames and saving those in
%   the format required for running the experiment, and setting some
%   params.  (This wouldn't really require a separate script for making an
%   experiment before running it, but we'll  do it anyway because it's
%   easier that way since the structure is already set up for doing this.)
%
%
% 
%
...
params.movie.duration = 0.2;      % set stimulus duration manually here (overwrite from initWordParams)
params.blankColor = 128;        % ISI uniform color e.g. 128 for gray
params.preStimBase = 1;         % how many seconds before stimuli start; allows MR signal to settle, not necessary for ecog
params.postStimBase = 1;        % how many seconds after last stimulus for a blank (in addition to last ITI)
params.RT = 10;                  % time after stimulus display to allow user to respond; ITI is in addition to the RT between trials
...
if ~loadParfileFlag  % then create parfile from scratch using a stimOrderFile
    ...
    meanITI = 500;                  % in millisecs; only used for poisson distribution way of generating ITIs
    minITI = 30;                   % useful for rand ITI generation; if using poisson, make sure to set very low for accurate poisson stats
    maxITI = 5000;                  % useful for rand ITI generation; if using poisson, set very high to allow accurate poisson distribution
...
if ~loadParfileFlag  % then we just assign conditions names, which we need to make sure will match
    ...
    params.condNames{1} = 'File-Object-SpellBack';
    params.condNames{2} = 'File-Object-Describe';    
...
for trialNum = 1:length(params.conditionOrder)
    ...
    % read in the right image to display when running    
    trialInfo.imagePath{1} = fullfile(params.baseDir,'stim','general','objectImages',[trialInfo.stimName '.png']);
    
    if trialInfo.condition == 1;
        trialInfo.imagePath{2} = fullfile(params.baseDir,'stim','general','objectImages','fix_red.jpg');
    else
        trialInfo.imagePath{2} = fullfile(params.baseDir,'stim','general','objectImages','fix_black.jpg');
    end
    numFrames = 2;
    ...
    trialInfo.seqtiming = [params.stimLength round(0.8*params.RT-params.stimLength)];
