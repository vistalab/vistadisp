function savedTrialsDir = TMSmakeMotionWordsRun(params,loadParfileFlag,thresh)
% This is the function that creates a new event related motion words
% experiment for TMS.  It is currently called from TMSmotionWordsEventRelated, only if
% the user has entered a run number that was not previously created/saved.
%
%  savedTrialsDir = TMSmakeMotionWordsRun(params,loadParfileFlag)
%
% ported from makeWordScanEventRelated by amr on Dec 9, 2009
% 
%
...
params.movie.duration = 1;      % set stimulus duration manually here (overwrite from initWordParams)
params.blankColor = 128;        % ISI uniform color e.g. 128 for gray
params.preStimBase = 1;         % how many seconds before stimuli start; allows MR signal to settle, not necessary for TMS
params.postStimBase = 2;        % how many seconds after last stimulus for a blank (in addition to last ITI)
params.RT = 1;                  % time after stimulus display to allow user to respond; ITI is in addition to the RT between trials
...
if ~loadParfileFlag  % then create parfile from scratch using a stimOrderFile
    ...
    meanITI = 4000;                  % in millisecs; only used for poisson distribution way of generating ITIs
    minITI = 2000;                   % useful for rand ITI generation; if using poisson, make sure to set very low for accurate poisson stats
    maxITI = 9000;                  % useful for rand ITI generation; if using poisson, set very high to allow accurate poisson distribution
...
if ~loadParfileFlag  % then we just assign conditions names, which we need to make sure will match
    ...
    params.condNames{1} = sprintf('MW_mot-%s.lum-0_Words_TMS-n100',num2str(thresh.mot));
    params.condNames{2} = sprintf('MW_mot-%s.lum-0_NW_TMS-n100',num2str(thresh.mot));
    params.condNames{3} = sprintf('MW_mot-0.lum-%s_Words_TMS-n100',num2str(thresh.lum));
    params.condNames{4} = sprintf('MW_mot-0.lum-%s_NW_TMS-n100',num2str(thresh.lum));
    params.condNames{5} = sprintf('PS_level-%s_Words_TMS-n100',num2str(thresh.PS));
    params.condNames{6} = sprintf('PS_level-%s_NW_TMS-n100',num2str(thresh.PS));
    
    params.condNames{7} = sprintf('MW_mot-%s.lum-0_Words_TMS-0',num2str(thresh.mot));
    params.condNames{8} = sprintf('MW_mot-%s.lum-0_NW_TMS-0',num2str(thresh.mot));
    params.condNames{9} = sprintf('MW_mot-0.lum-%s_Words_TMS-0',num2str(thresh.lum));
    params.condNames{10} = sprintf('MW_mot-0.lum-%s_NW_TMS-0',num2str(thresh.lum));
    params.condNames{11} = sprintf('PS_level-%s_Words_TMS-0',num2str(thresh.PS));
    params.condNames{12} = sprintf('PS_level-%s_NW_TMS-0',num2str(thresh.PS));
    
    params.condNames{13} = sprintf('MW_mot-%s.lum-0_Words_TMS-80',num2str(thresh.mot));
    params.condNames{14} = sprintf('MW_mot-%s.lum-0_NW_TMS-80',num2str(thresh.mot));
    params.condNames{15} = sprintf('MW_mot-0.lum-%s_Words_TMS-80',num2str(thresh.lum));
    params.condNames{16} = sprintf('MW_mot-0.lum-%s_NW_TMS-80',num2str(thresh.lum));
    params.condNames{17} = sprintf('PS_level-%s_Words_TMS-80',num2str(thresh.PS));
    params.condNames{18} = sprintf('PS_level-%s_NW_TMS-80',num2str(thresh.PS));
    
    params.condNames{19} = sprintf('MW_mot-%s.lum-0_Words_TMS-160',num2str(thresh.mot));
    params.condNames{20} = sprintf('MW_mot-%s.lum-0_NW_TMS-160',num2str(thresh.mot));
    params.condNames{21} = sprintf('MW_mot-0.lum-%s_Words_TMS-160',num2str(thresh.lum));
    params.condNames{22} = sprintf('MW_mot-0.lum-%s_NW_TMS-160',num2str(thresh.lum));
    params.condNames{23} = sprintf('PS_level-%s_Words_TMS-160',num2str(thresh.PS));
    params.condNames{24} = sprintf('PS_level-%s_NW_TMS-160',num2str(thresh.PS));
    
    params.condNames{25} = sprintf('MW_mot-%s.lum-0_Words_TMS-260',num2str(thresh.mot));
    params.condNames{26} = sprintf('MW_mot-%s.lum-0_NW_TMS-260',num2str(thresh.mot));
    params.condNames{27} = sprintf('MW_mot-0.lum-%s_Words_TMS-260',num2str(thresh.lum));
    params.condNames{28} = sprintf('MW_mot-0.lum-%s_NW_TMS-260',num2str(thresh.lum));
    params.condNames{29} = sprintf('PS_level-%s_Words_TMS-260',num2str(thresh.PS));
    params.condNames{30} = sprintf('PS_level-%s_NW_TMS-260',num2str(thresh.PS));
    
    params.condNames{31} = sprintf('MW_mot-%s.lum-0_Words_TMS-880',num2str(thresh.mot));
    params.condNames{32} = sprintf('MW_mot-%s.lum-0_NW_TMS-880',num2str(thresh.mot));
    params.condNames{33} = sprintf('MW_mot-0.lum-%s_Words_TMS-880',num2str(thresh.lum));
    params.condNames{34} = sprintf('MW_mot-0.lum-%s_NW_TMS-880',num2str(thresh.lum));
    params.condNames{35} = sprintf('PS_level-%s_Words_TMS-880',num2str(thresh.PS));
    params.condNames{36} = sprintf('PS_level-%s_NW_TMS-880',num2str(thresh.PS));
...
if ~loadParfileFlag
    ...
    meanITI = meanITI/10;  minITI = minITI/10;  maxITI = maxITI/10;  %chunk into 10ms bins
    params.ITIs = setITIs(methodForITIs,numITIs,meanITI,minITI,maxITI);
    params.ITIs = params.ITIs/100; % convert from ms to secs
end
...
save(paramInfoFile,'params','methodForITIs','meanITI','minITI','maxITI','stimOrderFile','parFilePath','thresh');
