...
%  Set up the colormap
backCol = 0;        % Index to the three color map entries
inFormCol = 1;
outFormCol = 2;
...
if(numSig<numDots)
    ...
    aFrame(dotPos(noiseDots)) = inFormCol;
end
...
if(numSig<numDots)  % the rest are noise dots, so make them the wrong luminance (or a random luminance?)
    aFrame(dotPos(numSig+1:numDots)) = outFormCol;
end
