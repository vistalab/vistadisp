function stim = tf_run(tfA, tfB, type, subj, ctrst, varargin);
%
%   stim = tf_run([tfA=1.5], [tfB=7.5], [type='L+M+S'], [subj], [contrast], [options]);
%
% Run an AB block-alternating temporal frequency stimulus, returning a structure 
% with stimulus and user behavioral data.
%
% INPUTS:
%
% tfA and tfB: temporal frequency, in Hz, of the two blocks.
%
...
% contrast in tfA and tfB. If omitted, they default to reasonable
...
if notDefined('type'),		type = 'L+M+S';			end
if notDefined('tfA'),		tfA = 1.5;				end
if notDefined('tfB'),		tfB = 7.5;				end
if notDefined('subj'),		subj = '';				end
if notDefined('ctrst'),		ctrst = [];				end

%% parse inputs
[type ctrst] = inputCheck(type, ctrst);
...
stim.fixationRad = 5;		% inner radius of disk in pixels
stim.nCycles = 6;			% # of temporal cycles
stim.secsPerBlock = 12;		% 2 blocks / cycle: tfA and tfB
stim.nCmap = 63;			% # colors in color map for each block (must be <127)
stim.runPriority = 7; 
stim.addSinePhase = 1;		% flag to alternate sine- and cosine-phase gratings
...
stim.temporalFrequencyHz = [tfA tfB];	
stim.contrastType = type;
stim.contrastLevel = ctrst;
...
for ii = 1:length(varargin)
    if ischar(varargin{ii})
        switch lower(varargin{ii})
			...
			case {'ncmap' 'ncolors'}, stim.nCmap = varargin{ii+1};
...
stim.display.backColorRGB = [.5 .5 .5];
% stim.display.quitProgKey = 'ESCAPE';
...
stim = createImages(stim);

%% combine the blocks, create a stimulus sequence
stim = createStimSequence(stim);
...
% /----------------------------------------------------------------/ %
function stim = createImages(stim);
%% Create the images and color maps for each block
fprintf('Creating images....');

%%%%% (1) make images for each individual block
...
% based on the desired temporal frequencies and the display refresh rate,
% figure out the number of unique color-contrast images for each block.
% we will set each image frame to last two screen refreshes (to prevent
% timing errors):
secsPerImgFrame = 2 / stim.display.frameRate;
stim.framesPerCycle = [1 1] ./ (secsPerImgFrame .* stim.temporalFrequencyHz);

% check that there are an integer number of frames per cycle for each
% block. If not (because of a discrepancy between the temporal frequencies
% and the display), warn the user, and update the temporal frequencies to
% match the nearest frames/cycle measure
for ii = 1:2
	if mod(stim.framesPerCycle(ii), 1) ~= 0
		% round the frames per cycle.
		stim.framesPerCycle(ii) = round(stim.framesPerCycle(ii));
		
		% what actual temporal frequency does this new frames/cycle value
		% reflect?
		newTF = 1 / (stim.framesPerCycle(ii) * secsPerImgFrame);
		
		% give the user a detailed warning.
		msg = sprintf(['The desired temporal frequence of %f Hz can not be ' ...
					   'achieved on this display. (%s refresh rate is %f Hz.) ' ...
					   'The actual temporal frequency of block %i has been ' ...
					   'adjusted to %f Hz. Update the display settings ' ...
					   'to avoid this adjustment.'], ...
					   stim.temporalFrequencyHz(ii), ...
					   stim.displayName, stim.display.frameRate, ii, newTF);
		warning( sprintf('[%s]: %s', mfilename, msg) );
		
		% update the temporal frequency in the stimulus structure.
		stim.temporalFrequencyHz(ii) = newTF;
	end
end


%% create cosine- and sine-phase images for block tfA
[imagesA cmapA] = colorContrastImages(stim.contrastType, stim.display, ...
									'contrast', stim.contrastLevel, ...
									'fixationPos', stim.fixationPos, ...									
                                    'nCmap', stim.nCmap, ...
                                    'nCycles', stim.nSpatialCycles, ...
                                    'nImages', stim.framesPerCycle(1), ...
									'fixationRad', stim.fixationRad, ...
                                    'sz', stim.imSizePixels);
imagesA2 = colorContrastImages(stim.contrastType, stim.display, ...
									'contrast', stim.contrastLevel, ...
									'fixationPos', stim.fixationPos, ...									
									'PhaseOffset', pi/2, ...
                                    'nCmap', stim.nCmap, ...
...
%% create cosine- and sine-phase images for block tfB								
[imagesB cmapB] = colorContrastImages(stim.contrastType, stim.display, ...
									'contrast', stim.contrastLevel, ...
									'fixationPos', stim.fixationPos, ...									
                                    'nCmap', stim.nCmap, ...
                                    'nCycles', stim.nSpatialCycles, ...
                                    'nImages', stim.framesPerCycle(2), ...
									'fixationRad', stim.fixationRad, ...
									'sz', stim.imSizePixels);
imagesB2 = colorContrastImages(stim.contrastType, stim.display, ...
									'contrast', stim.contrastLevel, ...
									'fixationPos', stim.fixationPos, ...									
									'PhaseOffset', pi/2, ...
                                    'nCmap', stim.nCmap, ...
...
%%%%% (2) making the cmap -- we reserve several colors:
...
%%%%% (3) combine the images -- because of the way we combined color maps
...
% /---------------------------------------------------------------------/ %
function stim = createStimSequence(stim);
%% Create stim.seq, stim.seqtiming, and stim.fixSeq:
%% The first field specifies which image to display ont the screen at each
%% time, the second specifies when (in secs after run start) to put up each
%% images, and the third provides an index into the set of fixation colors
%% for the fixation task.

%% build the sequence (seq)
% build sequence for block A
rng = 1:stim.framesPerCycle(1);
seqA = buildBlockSequence(stim, rng, 1); 

% build sequence for block B
rng = stim.framesPerCycle(1)+1:sum(stim.framesPerCycle, 2);
seqB = buildBlockSequence(stim, rng, 2);  

% the full sequence is multiple cycles of A/B sequence alternation:
singleCycle = [seqA seqB];
stim.seq = repmat(singleCycle, [1 stim.nCycles]);
stim.seq(1) = -1;  % set the color map

%% figure out the timing (seqtiming)
% % timing for block A
imagePeriod = 2 / stim.display.frameRate;
stim.seqtiming = [0:length(stim.seq)-1] .* imagePeriod;

%% figure out the fixation sequence (fixSeq)
stim.fixSeq = ones(size(stim.seq));  % dummy placeholder
								
% add fixation point, task sequence to images
...
% /---------------------------------------------------------------------/ %
function seq = buildBlockSequence(stim, rng, blockNum);
...
reversalsPerBlock = stim.temporalFrequencyHz(blockNum) * stim.secsPerBlock;
seq = repmat(rng, [1 reversalsPerBlock]);

%% compute parameters needed to reak up block into several "trials"
...
framesPerBlank = framesPerSec * stim.secsBlank;
...
% sine-phase gratings.
if stim.addSinePhase==1
	% index offset to point to sine images
	sineOffset = sum( stim.framesPerCycle ); 
	
	trialFrames = framesPerSec * stim.secsPerTrial;

	% randomly determine which trials will use sine phase
...
%% add blank periods to the end of each trial
...
% bin:
startIndex = floor( linspace(1, length(seq), nTrials+1) );
startIndex = startIndex(2:end) - framesPerBlank + 1;

offset = meshgrid(0:framesPerBlank-1, 1:length(startIndex));
blankIndices = repmat(startIndex(:), [1 framesPerBlank]) + offset;
blankIndices = blankIndices(:);

% point the blank frames to the index for a grayscale screen 
seq(blankIndices) = stim.framesPerCycle(1) + 1; 
