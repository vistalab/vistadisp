...
% renobowen@gmail.com 2010
    ...
    for i = 1:nDirs
        ...
        for ii = 1:nROIs
            ...
            save(filename, 'data', 'analysis');
            
            % Saving out mean TCs into a few separate files
            nTrialSets = size(allTrials{i},2);
            meanTcs = cell(1, nTrialSets);
            for iii = 1:nTrialSets
                struct = classify_GetMeanTCs(filename, allTrials{i}{iii}{1});
                meanTcs{iii} = {struct, allTrials{i}{iii}{1}};
            end
            save(filename, 'data', 'analysis', 'meanTcs');
        end
